...
const GlobalState = (props) => {
  ...
  const setYhteystiedot = async (uusiYhteystieto) => {
    try {
      const res = await axios
        .post(`http://localhost:3001/lisaa`, uusiYhteystieto)
        .then((res) => {
  ...
  const setYhteystieto = async (id, paivitettyYhteystieto) => {
    try {
      const res = await axios
        .put(`http://localhost:3001/henkilot/${id}`, paivitettyYhteystieto)
        .then((res) => {
